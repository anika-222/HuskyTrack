#!/usr/bin/env python3
"""
UW CSE Course Scraper - Final Version
Extracts only Computer Science courses (CSE prefix) from the UW CS courses page
"""
...
class UWCourseScraper:
    ...
    def extract_prerequisites(self, text: str) -> List[str]:
        ...
        # Extract course codes from prerequisites - only CSE courses
        course_codes = []
        for prereq in prereqs:
            codes = re.findall(r'CSE\s*\d{3}[A-Z]?', prereq)
    ...
    def save_to_csv(self, filename: str = "cse_courses.csv"):
        """Save CSE courses to CSV file"""
        if not self.courses:
    ...
    def save_to_json(self, filename: str = "cse_courses.json"):
